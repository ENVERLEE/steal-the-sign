"""STEAL THE SIGN : KICE 교재 파이프라인
  ...
  python run.py check        검사만: validate → created → check → verify → report
  python run.py build        조판만: figs → build → layout → report
  python run.py created      창작 문제은행 검문: validate → created → report
  python run.py <단계>       validate | created | check | verify | figs | layout | report 중 하나

  옵션: --book PATH  --created DIR  --out DIR  --recheck(창작 전체 재검사)  --force(검사 오류가 있어도 조판)
"""
...
PLANS = {
    "all": ["validate", "created", "check", "verify", "figs", "build", "layout", "report"],
    "check": ["validate", "created", "check", "verify", "report"],
    "build": ["figs", "build", "layout", "report"],
    "created": ["validate", "created", "report"],
}
NEEDS_BOOK = {"check", "verify", "figs", "build"}
GATE = {"validate", "created", "check", "verify"}  # 이 단계에 오류가 있으면 조판하지 않는다
...
def main(argv=None) -> int:
    ...
    ctx = Context(book_path=args.book, created_dir=args.created, out_dir=args.out)
