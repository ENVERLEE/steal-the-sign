...
`scripts/tex.py`는 Playwright 헤드리스 브라우저에 이 파일을 올려 `katex.renderToString`으로 수식을 미리 조판한다.
결과 HTML에는 `katex.min.css`를 템플릿의 KaTeX SLOT 자리에 `<style>`로 넣고, woff2 글꼴은 base64로 내장한다
(결과물 `out/book.html` 한 파일로 완결).
